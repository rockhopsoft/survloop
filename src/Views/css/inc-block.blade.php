...
@if (sizeof($node->colors) > 0)
    <style>
    @if (isset($node->colors["blockHeight"]) 
        && trim($node->colors["blockHeight"]) != '' 
        && trim($node->colors["blockHeight"]) != 'auto')
        #blockWrap{{ $nIDtxt }} {
            height: @if ($node->colors["blockHeight"] == 'h100') 95% 
                @else {{ substr($node->colors["blockHeight"], 1) }}% @endif ;
            padding-bottom: 0px;
            overflow: visible;
        }
        #blockWrap{{ $nIDtxt }} .nodeWrap, 
        #blockWrap{{ $nIDtxt }} .nodeWrapError { 
            height: 100%;
            padding-top: 20px;
        }
    @endif
    @if (isset($node->colors["blockAlign"]) 
        && trim($node->colors["blockAlign"]) != '')
        #blockWrap{{ $nIDtxt }}, #blockWrap{{ $nIDtxt }} .nodeWrap, 
        #blockWrap{{ $nIDtxt }} .nodeWrapError { 
            text-align: {{ $node->colors["blockAlign"] }}; 
        }
    @endif
    @if (isset($node->colors["blockImg"]) 
        && trim($node->colors["blockImg"]) != '')
        @if (!isset($node->colors["blockImgFix"]) 
            || $node->colors["blockImgFix"] != 'P')
            /* CSS for other than iOS devices */
            @supports not (-webkit-overflow-scrolling: touch) {
                #blockWrap{{ $nIDtxt }} {
                    background: url('{{ $node->colors["blockImg"] }}') 
                    @if (isset($node->colors["blockBG"]) 
                        && trim($node->colors["blockBG"]) != '') 
                        {{ $node->colors["blockBG"] }} 
                    @endif ;
                    @if (isset($node->colors["blockImgType"]) 
                        && trim($node->colors["blockImgType"]) == 'tiles')
