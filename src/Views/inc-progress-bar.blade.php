...
<div id="navDesktop">
    ...
    <div>
        ...
        @forelse ($majorSections as $maj => $majSect)
            <li class=" @if ($maj == $currMajorSection) active @elseif (in_array($maj, $sessMajorsTouched)) completed @endif ">
                ...
                <div class="subNav">
                    <div class="row">
                        <div class="col-md-1 snLabel relDiv">
                            <div class=" @if ($maj == $currMajorSection) stepNum2active @elseif (in_array($maj, $sessMajorsTouched)) stepNum2complete @else stepNum2 @endif ">{{ (1+$maj) }}</div>
                        </div>
                        <div class="col-md-11">
                            <ul class="nav nav-pills nav-justified nav-wizard">
                            @forelse ($minorSections[$maj] as $min => $minSect)
                                <li class=" @if ($maj == $currMajorSection && $min == $currMinorSection) active @elseif (in_array($min, $sessMinorsTouched[$maj])) completed @endif ">
                                    @if ($min > 0) <div class="nav-wedge"></div> @endif
                                    @if ( (!isset($sessMinorsTouched[$maj]) || !in_array($min, $sessMinorsTouched[$maj])) && ($maj != $currMajorSection || $min != $currMinorSection) )
                                        <a href="javascript:void(0)" ><nobr>{{ $minSect[1] }}</nobr></a>
                                    @else 
                                        <a href="/u/{{ $allNodes[$minSect[0]]->nodeRow->NodePromptNotes }}" ><nobr>
                                        @if (in_array($min, $sessMinorsTouched[$maj]) && ($maj != $currMajorSection || $min != $currMinorSection)) <i class="fa fa-check"></i> @endif 
                                        {{ $minSect[1] }}</nobr></a>
                                    @endif
...
<div id="navMobile">
    ...
    <div class="f16">
        <a id="navMobBurger1" class="disIn mR10" href="javascript:void(0)"><i class="fa fa-caret-right f16" aria-hidden="true"></i></a>
        <a id="navMobBurger2" class="disNon mR5" href="javascript:void(0)"><i class="fa fa-caret-down f16" aria-hidden="true"></i></a>
        <b>{{ $majorSections[$currMajorSection][1] }}</b>
        @if (sizeof($minorSections[$currMajorSection]) > 0 && isset($minorSections[$currMajorSection][$currMinorSection]))
            ...
            <div class="brdBotBluL pT5 pB5">
                ...
                @forelse ($minorSections[$maj] as $min => $minSect)
                    @if (in_array($min, $sessMinorsTouched[$maj]))
                        <a href="/u/{{ $allNodes[$minSect[0]]->nodeRow->NodePromptNotes }}" class=" @if ($maj == $currMajorSection && $min == $currMinorSection) navMobActive @else navMobDone @endif " >
