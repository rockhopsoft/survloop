<!-- resources/views/vendor/survloop/inc-report-deets-cols.blade.php -->
<div class="w100" @if (isset($nID)) id="repNode{{ $nID }}" @endif >
    ...
    @if (isset($deetCols) && sizeof($deetCols) > 0)
        @if (in_array($GLOBALS["SL"]->x["pageView"], ['pdf', 'full-pdf'])) <table border=0 class="w100 brdNo"><tr>
        @else <div class="row"> @endif
        @foreach ($deetCols as $i => $deets) 
            @if (in_array($GLOBALS["SL"]->x["pageView"], ['pdf', 'full-pdf']))
                <td class="vaT @if (sizeof($deetCols) == 2) w50 @elseif (sizeof($deetCols) == 3) w33 
            ...
            @else <div class="col-md-{{ $GLOBALS['SL']->getColsWidth(sizeof($deetCols)) }}"> @endif
                ...
                @foreach ($deets as $j => $deet)
                    @if (isset($deet[0]) && trim($deet[0]) != '')
                       ...
                       {!! view('vendor.survloop.inc-report-deets-row', [ "i" => $cnt, "deet" => $deet ])->render() !!}
                    @endif
                @endforeach
                </table>
