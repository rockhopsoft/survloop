...
<?php /*@if (trim($upRow->UpDesc) != '') <div class="fPerc133">{{ $upRow->UpDesc }}</div> @endif */?>
                        <div class="slGrey pT5">
                            @if (isset($GLOBALS["SL"]->treeSettings["uploads-public"]) 
                                ...
                                @if (in_array($upRow->up_privacy, ['Public', 'Open'])) Public @else Private @endif
                            @endif
                    ...
                    <div id="up{{ $upRow->up_id }}InfoEdit" class="disNon pB10">
                        ...
                        <div class="nFld mT0">
                            <input type="text" class="form-control form-control-lg ntrStp slTab" 
                                ...
                                value="{{ $upRow->up_title }}" {!! $GLOBALS["SL"]->tabInd() !!}>
                        </div>
                        ...
                        @if (sizeof($uploadTypes) > 1)
                            ...
                            <div class="nPrompt"><label for="up{{ $upRow->up_id }}EditTypeID">Upload Type:</label></div>
                            <div class="nFld mT0"><select name="up{{ $upRow->up_id }}EditType" 
                                id="up{{ $upRow->up_id }}EditTypeID" class="form-control form-control-lg ntrStp slTab"
                                {!! $GLOBALS["SL"]->tabInd() !!}>
                        ...
                        @if (isset($GLOBALS["SL"]->treeSettings["uploads-public"]) 
                            && intVal($GLOBALS["SL"]->treeSettings["uploads-public"][0]) > 0)
                            <div class="nodeHalfGap"></div>
                            <div class="nPrompt"><label for="up{{ $upRow->up_id }}EditPrivacyID">Privacy:</label></div>
                            <div class="nFld mT0"><select name="up{{ $upRow->up_id }}EditPrivacy" 
                                id="up{{ $upRow->up_id }}EditPrivacyID" class="form-control form-control-lg ntrStp slTab"
                                {!! $GLOBALS["SL"]->tabInd() !!}>
                                <option value="Public" @if ($upRow->up_privacy == 'Public') SELECTED @endif 
                                    >Public: Visible to whole world</option>
                                <option value="Private" @if ($upRow->up_privacy != 'Public') SELECTED @endif 
