...
<div class="pT15">
    <a id="hidivBtnMonthly{{ $nIDtxt }}"
        class="hidivBtn" href="javascript:;"
        ><i class="fa fa-calculator mR5" aria-hidden="true"></i>
        Monthly Calculator</a>
            ...
            @for ($i = 1; $i < 13; $i++)
                <div class="col-md-3 col-sm-4 col-6 pB20">
                    ...
                    <input type="number" class="form-control"
                        name="month{{ $nIDtxt }}ly{{ $i }}"
                        id="month{{ $nIDtxt }}ly{{ $i }}ID"
                        value="{{ (($presel[($i-1)]) ? $presel[($i-1)] : '') }}">
                </div>
            @endfor
        </div>
        <a id="monthlyCalcTot{{ $nIDtxt }}" href="javascript:;"
