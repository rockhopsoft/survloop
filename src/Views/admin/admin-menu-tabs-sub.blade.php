@forelse ($adminNav as $i => $nav)
    @if (isset($nav[0]) && isset($nav[4]) && sizeof($nav[4]) > 0 && $currNavPos[0] == $i)
        @foreach ($nav[4] as $j => $nA)
            @if (isset($nA[4]) && sizeof($nA[4]) > 0 && $currNavPos[0] == $i && $currNavPos[1] == $j)
                @foreach ($nA[4] as $k => $nB)
                    @if (isset($nB[4]) && sizeof($nB[4]) > 0 && $currNavPos[0] == $i 
                        && $currNavPos[1] == $j && $currNavPos[2] == $k)
                        <ul class="nav">
                            <li class="pL20">&nbsp;</li>
