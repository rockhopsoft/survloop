...
<div id="adminAbout" class="jumbotron @if ($showAbout) disBlo @else disNon @endif ">
    ...
    Most nodes in this tree represents either a single question to ask the user, instructions, or a <nobr>page 
    (<i class="fa fa-file-text-o"></i>)</nobr> which groups multiple child questions on one screen.
    </p><p>
    ...
    A few nodes are "branches" (<span class="f12"><i class="fa fa-share-alt"></i></span>) which wrap multiple pages 
    ...
    Some nodes are marked as the root node of a SurvLoop (<span class="f12"><i class="fa fa-refresh"></i></span>). 
    The children of a SurvLoop node will loop through multiple records of one collection of data. They serve one or 
    more questions, pages, or whole branches, for each record. They can be setup to allow the user to add new records 
