...
@forelse ($adminNav as $i => $nav)
    @if (isset($nav[0]))
        ...
        @else
            <div class="admMenuTier1"><a href="{!! $nav[0] !!}" data-toggle="collapse" 
                class="admMenuTier1Lnk @if ($currNavPos[0] == $i) active @endif
                @if ($GLOBALS['SL']->isAdmMenuHshoo($nav[0])) hshoo @endif
                primeNav" data-target="#subA{{ $i }}"
            ...
            @foreach ($nav[4] as $j => $nA)
                <div class="admMenuTier2"><a href="{!! $nA[0] !!}" @if ($nA[3]%3 == 0) target="_blank" @endif class="admMenuTier2Lnk
                @if ($currNavPos[0] == $i && $currNavPos[1] == $j) tier2active @endif
                @if ($GLOBALS['SL']->isAdmMenuHshoo($nA[0]))
                    hshoo" id="admLnk{{ substr($nA[0], 1+strpos($nA[0], '#'))}}
                @else " @endif > 
