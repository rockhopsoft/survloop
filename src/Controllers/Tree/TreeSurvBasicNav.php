...
class TreeSurvBasicNav extends TreeSurvProgBar
{
    ...
    */
        ...
        if (intVal($nID) > 0 && isset($this->allNodes[$nID])) {
            ...
            if (isset($this->allNodes[$nID]->nodeRow->node_prompt_notes)
                ...
                if ($this->hasREQ
                    ...
                    $url = $this->getCurrNodeUrl($nID);
                    $title = ((trim($title) != '') ? $title . ' - ' : '')
                        . $GLOBALS["SL"]->sysOpts["site-name"];
                    $this->v["currPage"] = [ $url , $title ];
                    $GLOBALS["SL"]->pushBrowserStateUrl($url , $title, $this->currNode());
                }
            }
        }
        return true;
    ...
    protected function getCurrNodeUrl($nID)
    {
        if (isset($this->allNodes[$nID])
            && isset($this->allNodes[$nID]->nodeRow->node_prompt_notes)) {
    ...
    {
        if (isset($this->sessInfo->sess_id)
            && $nID > 0
