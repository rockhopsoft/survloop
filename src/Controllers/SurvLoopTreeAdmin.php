...
class SurvLoopTreeAdmin extends SurvFormTree
{
	...
	public function adminNodeEdit($nodeIN, Request $request, $currPage = '') 
	{
		...
		if ($this->REQ->has('sub') && $this->canEditTree)
		{
			...
			else
			{
				...
				$node->nodeRow->NodeCharLimit 		= trim($this->REQ->input('nodeCharLimit'));
				...
				elseif ($this->REQ->nodeType == 'loop')
				{
					...
					if ($this->REQ->has('stepLoop') && intVal($this->REQ->input('stepLoop')) == 1)
					{
						...
						if ($this->REQ->has('stepLoopDoneField') && trim($this->REQ->input('stepLoopDoneField')) != '')
						{
							$GLOBALS["DB"]->dataLoops[$loop]->DataLoopDoneFld = trim($this->REQ->input('stepLoopDoneField'));
						}
					}
				...
				else // other normal response node
				{
					...
					foreach ($opts as $o)
					{
						...
						elseif ($node->nodeRow->NodeOpts%$o == 0) $node->nodeRow->NodeOpts = $node->nodeRow->NodeOpts/$o;
					}
	...
	protected function adminResponseNodeStatsTxt($res, $nodeFinalCnt, $nodeAttemptsCnt, $nodeFinalVals, $nodeAttempts, $nodeSess)
	{
		...
		if (isset($nodeFinalVals[$res])) $stats .= '<b>' . number_format(100*$nodeFinalVals[$res]/$nodeFinalCnt, 1).'%</b> <span class="f12">of ' . sizeof($nodeSess).' Final Submissions</span>; ';
		else $stats .= '<b>0%</b> <span class="f12">of ' . sizeof($nodeSess).' Final Submissions</span>; ';
		if (isset($nodeAttempts[$res])) $stats .= number_format(100*$nodeAttempts[$res]/$nodeAttemptsCnt, 1).'% <span class="f12">of All ' . $nodeAttemptsCnt . ' Attempts</span>; ';
		else $stats .= '0% <span class="f12">of All Attempts</span>; ';
		return $stats . '</span>';
	}
	...
	protected function adminResponseNodeStats($tierNode = array(), $tierDepth = 0)
	{
		...
		if (sizeof($tierNode) > 0 && $tierNode[0] > 0)
		{
			...
			if ($this->hasNode($nID))
			{
				...
				if (sizeof($nodeSaves) > 0)
				{
					foreach ($nodeSaves as $save)
					{
						...
						if (!isset($nodeSess[$save->NodeSaveSession]))
						{
							...
							{
								if (!isset($nodeFinalVals[strtolower($res)])) $nodeFinalVals[strtolower($res)] = 0;
								$nodeFinalVals[strtolower($res)]++;
								$nodeFinalCnt++;
							}
						}
						else $nodeSess[$save->NodeSaveSession]++;
						foreach ($responses as $j => $res)
						{
							if (!isset($nodeAttempts[strtolower($res)])) $nodeAttempts[strtolower($res)] = 0;
				...
				$nodeAJAX = '';
				$retVal .= '<a name="n'.$nID.'"></a><div class="basicTier' . (($tierDepth < 10) ? $tierDepth : 9) . '">'
					. ((!$this->allNodes[$nID]->isPage()) ? '<span class="slBlueDark f22" title="(on parent\'s page)"><i class="fa fa-angle-double-up"></i></span> ' : '') 
					. (($this->allNodes[$nID]->isLoopRoot()) ? '<span class="f22 slBlueDark"><i class="fa fa-refresh"></i> </span>' : '')
					. (($this->allNodes[$nID]->isDataManip()) ? '<span class="f22 slBlueDark"><i class="fa fa-database"></i> </span>' : '')
					. '<span class="slBlueDark f22"><span class="f16">#'.$nID.'</span><i class="fa fa-chevron-right"></i></span> ';
				if ($this->allNodes[$nID]->isBranch())
				{
					$retVal .= '<span class="f20"><span class="gry9"><i class="fa fa-share-alt"></i> <i>' . (($nID == $this->rootID) ? 'Tree Root Node' : 'Section Branch') . ':</i></span> <b>' 
					. $this->allNodes[$nID]->nodeRow->NodePromptText . '</b></span>'
					. '<div class="pT5"><a href="#n'.$nID.'" id="adminNode'.$nID.'Expand" class="adminNodeExpand slBlueLight noUnd"><i class="fa fa-expand fa-flip-horizontal"></i></a></div>';
				}
				else
				{ // non-branch nodes
					...
					if (sizeof($this->allNodes[$nID]->nodeResponses) > 0)
					{
						...
						foreach ($this->allNodes[$nID]->nodeResponseVals as $j => $res)
						{
							...
							{
								if (sizeof($this->allNodes[$nID]->nodeResponsesShowKids) > 0 && in_array($j, $this->allNodes[$nID]->nodeResponsesShowKids))
								{
									$retVal .= '<li class="mT5 mB20"><span class="f18"><b>'.strip_tags($res).'</b></span> 
									<i class="fa fa-code-fork fa-flip-vertical" title="Children displayed if selected"></i><br />' . $stats . '</li>';
								}
								else $retVal .= '<li class="mT5 mB20"><span class="f18"><b>' . strip_tags($res) . '</b></span><br />' . $stats . '</li>';
							}
						}
						$retVal .= '</ul>';
					}
				...
				if (sizeof($tierNode[1]) > 0)
				{ 
					$retVal .= '<div id="nodeKids' . $nID . '" class="dis' . ((session()->get('adminOverOpts')%2 == 0 || $nID == $this->rootID) ? 'Blo' : 'Non') . '">';
	...
	public function checkTreeSessOpts()
	{
		...
		if ($this->REQ->has('all')) {
			if (session()->get('adminOverOpts')%2 > 0) session()->put('adminOverOpts', (2*session()->get('adminOverOpts')));
		}
		elseif (session()->get('adminOverOpts')%2 == 0) session()->put('adminOverOpts', (session()->get('adminOverOpts')/2));
		return true;
	}
	...
	protected function adminBasicDropdownNode($tierNode = array(), $tierDepth = 0, $preSel = -3)
	{
		...
		{ 
			...
			if ($this->hasNode($nID))
			{
				...
				$retVal .= '<option value="'.$nID.'" ' . ((intVal($preSel) == $nID) ? 'SELECTED' : '') . ' >' . $indent 
					. ((strlen($nodeName) > 70) ? substr($nodeName, 0, 70).'...' : $nodeName) . '</option>';
				if (sizeof($tierNode[1]) > 0)
				{
					foreach ($tierNode[1] as $next)
					{
